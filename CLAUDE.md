...
3. **APPROVED 상태 데이터만 집계·분석에 사용.** AI 추출값은 CANDIDATE로 시작, 사람 승인 전엔 공식 숫자에 반영 금지.
   단 이것은 **전수 승인 의무가 아니다** — 승인 대상은 실제로 쓰이는 값뿐이고, 쓰이지 않는 후보는 CANDIDATE로 남아도 무해하다.
   가설 DRAFT 생성과 검토 큐 정렬에는 CANDIDATE를 포함한 **잠정 집계**를 쓴다(유일한 예외, docs/02 §5.6).
   화면에는 **잠정과 공식 두 숫자를 나란히** 놓아 이 구분을 눈으로 보이게 한다.
