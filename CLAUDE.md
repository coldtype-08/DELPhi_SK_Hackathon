...
① Data Contract v0.1 승인 → ② 합성 원석(영문 하이라이트·면담록) 구조화 — HCP 블록 분리 포함 + 사람 검토·승인 → ③ Field 앱으로 신규 면담 수집(한국어: 동의→전사→AE 분기→카드 승인 — 영문 원석과 같은 집계에 합산되는 순간이 다국어 시연 포인트) → ④ Screen 다중 에이전트 교차검증(PubMed·CT.gov·openFDA) → ⑤ Board 심의 + CEO 권고 + 사람 승인 — 승인 즉시 후속 질문이 **Action Item으로 Field 체크리스트에 내려가고**, 다음 면담이 그 질문을 참조해 수집된다(실행 루프가 닫히는 장면) → ⑥ Schema Change Proposal 승인 → Contract v0.2 → Field 입력 폼이 실제로 바뀜(구조 루프가 닫히는 장면).
