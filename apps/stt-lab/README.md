...
1. **사람이 직접 읽는다** — 음성 파일이 필요 없다. 「동시 시작」 후
...
2. **「동시 시작」** → 직접 말하거나 스피커로 음성을 틀고 → **「중지」**
3. 탭을 옮겨 다니며 같은 소리에 대한 세 결과를 비교합니다

마이크는 **항상 전처리(에코 제거·잡음 억제·자동 게인) 없이** 동작합니다 — 전처리는 화상통화용이라
STT에는 원음이 낫고, 켜두면 스피커로 재생한 소리를 에코로 판정해 지워버립니다. 별도 설정은 없습니다.
