...
**위험 등급 H/M/L은 자동 검증 3종의 결과다.** 등급만 표시하면 아무도 읽지 못하므로 **화면에 뜻을 함께 적는다.**

| 검증 | 내용 | 실패 시 |
|---|---|---|
| ① 원문 일치 | 인용 문자열이 원문과 문자 단위로 일치하나 | **저장 자체가 거부**된다(절대 규칙 #2) → 큐에 못 들어옴 |
| ② 용어 매핑 | 표현이 스키마 허용값으로 매핑됐나 | `UNSPECIFIED` 낙착 → **H** |
| ③ 파생 규칙 | 필수·조건부 필드를 채웠나 (`signal_type=TREATMENT_BARRIER` → `barrier_type` required_if) | 미충족 → **H** |

셋 다 통과 = M 또는 L. **H로 큐에 올라오는 것은 언제나 ②나 ③이다** — ①이 실패한 것은 애초에 저장되지 않았으므로. 화면에 이 문장을 그대로 적어 "H가 뭔가"를 묻지 않게 한다.

②의 `UNSPECIFIED` 낙착이 반복되면 그 자체가 **SCP 후보 신호**다 (§7의 `unmapped_terms` 조건). 검토 화면에서 SCP 제안으로 바로 연결한다.
