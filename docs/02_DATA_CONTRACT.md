...
**화면은 묶음 검토여야 한다 (08/22).** 카드 1장씩 넘기는 UI는 "검토량 ∝ 데이터량"을 그림으로 다시 만든다 — 문서 1건에서 12행이 나오는데 12번 눌러야 하면, 없애려던 노동이 "문서를 읽는 일"에서 "카드를 누르는 일"로 이름만 바뀐다.

| 규칙 | 내용 |
|---|---|
| 기본 단위 | **가설 1건이 딛고 선 근거 묶음** (예: HYP-003의 34건 중 미검토 13건). 전체 큐를 훑는 화면이 아니다 |
| 기본 선택 | 자동 검증 3종을 모두 통과한 행은 **체크된 상태로 시작**. 실패한 행만 체크가 풀린 채 맨 위 |
| 승인 동작 | **한 번의 클릭 = 선택된 전부** (`POST /claims/batch`, docs/04 §2). 백엔드를 N번 호출하지 않는다 |
| 체크 해제 | 해제한 행은 `CANDIDATE`로 남는다. 반려가 아니다 — 쓰이지 않을 뿐이다 |

**검토등급 H/M/L은 자동 검증 3종의 결과다** (§5의 A·B·C와 같은 것). **H가 안전하고 L이 위험하다** — 등급만 표시하면 방향을 거꾸로 읽으므로 **화면에 뜻을 함께 적는다.**

| 검증 | 내용 | 실패 시 |
|---|---|---|
| ① 원문 일치 | 인용 문자열이 원문과 문자 단위로 일치하나 | **저장 자체가 거부**된다(절대 규칙 #2) → 큐에 못 들어옴 |
| ② 용어 매핑 | 표현이 스키마 허용값으로 매핑됐나 | `UNSPECIFIED` 낙착 → **M** |
| ③ 파생 규칙 | 필수·조건부 필드를 채웠나 (`signal_type=TREATMENT_BARRIER` → `barrier_type` required_if) | 미충족 → **L** |

셋 다 통과 = **H**. 큐는 **L → M → H 순**으로 올린다(위험한 것부터 사람 눈에 넣는다). 따라서 **큐 위쪽에 오는 것은 언제나 ②나 ③에 걸린 행이다** — ①이 실패한 것은 애초에 저장되지 않았으므로 큐에 없다. 화면에 이 문장을 그대로 적어 "L이 뭔가"를 묻지 않게 한다.

②에 걸려 `M`으로 떨어지는 일(=`UNSPECIFIED` 낙착)이 반복되면 그 자체가 **SCP 후보 신호**다 (§7의 `unmapped_terms` 조건). 검토 화면에서 SCP 제안으로 바로 연결한다.
