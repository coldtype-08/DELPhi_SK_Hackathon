...
- 08/22 인혁(Claude 세션): **STT 3종 비교 랩 + 평가 대본 3종 — 8/24 실기 테스트(DECISIONS 08/21 [소정])용 판정 도구.** `apps/stt-lab`(포트 3002, 로컬 전용·배포 안 함 / Next 16.3.1·React 19.2.8, Field와 동일 스택·페이퍼 라이트 토큰) — `lib/stt/` provider 3종(Soniox `stt-rt-v5`·Deepgram `nova-3`/`nova-3-medical`·Gladia `solaria-1`) + 오디오(마이크 AudioWorklet·WAV→16kHz PCM16) + 핵심 토큰 채점. 키는 화면 입력 후 브라우저 localStorage라 백엔드 불필요. **벤더 확인은 SDK 소스·정본 스펙 기준(마케팅 문구 아님)** — 프록시에서 세 벤더 문서가 모두 403이라 GitHub의 SDK·OpenAPI·AsyncAPI를 직접 읽었다. `nova-3-medical`은 한국어 미지원 확인(→ **의료 특화와 한국어는 동시 불가**), Gladia 실시간 화자분리는 정본 스펙에 없어 `diarization` 실송해 422/201로 실측하게 배선. **평가 대본 3종 확정(전부 한국어)**: P1 = HYP-003(PGTC) 신규 7턴(S7+AE+자료요청+PII를 한 대본에 — 시연 장면 5개가 한 번에) / D1 = Field 앱 대본 7턴 그대로 / T1 = D1 + 영어 용어·전화번호. 대본과 `P1_reference.txt`가 어긋나면 음성 생성이 즉시 실패하는 드리프트 가드를 넣었다(같은 자산을 여러 곳에서 쓰므로). **비교 방식을 「3종 동시 실행」으로 바꿨다** — 마이크 하나를 세 세션에 동시에 흘려 한 번의 발화·재생을 셋이 같이 듣게 함(파일 업로드는 마이크·공기를 건너뛰어 실전과 다른 조건이므로 배선 확인용으로 격하). 스피커 재생 캡처 시 브라우저 에코 제거를 꺼야 한다는 것도 여기서 발견. **espeak-ng 합성음은 들어보고 판정에 못 쓴다고 결론** — 판정 음성은 사람이 직접 만든다(상용 TTS는 프록시 차단). 3종 비교를 `scripts/stt_eval/SERVICE_COMPARISON.md`에 팀 공용으로 정리([확인]/[추정]/[실측] 표기 + 탈락 후보 사유). 빌드·타입체크·린트 통과(실 STT 호출은 키 없어 미검증) — 실행법 `apps/stt-lab/README.md` (다음: 키 발급 → 8/24 실측 → 결과 소정에게 · provider 확정 후 `lib/stt/`를 Field로 이식 · P1 대본은 소정·건태 확인 대상)
