...
- 08/22 인혁(Claude 세션): **STT 3종 비교 랩 완성 + 첫 실측 — Gladia 탈락, Deepgram 생존, Soniox 충전 대기.** 랩: 3종 동시 실행(마이크 하나→세 소켓) + 3열 라이브 뷰(서비스별 전사·토큰별 채점·첫 응답 ms 나란히) + Gemini TTS 대본→음성 섹션(화자 2인 한 호출, 모델 목록은 벤더에서 수신) + 전사를 확정 문장 단위로(중간 결과는 한 줄 교체). 대본: D1(Field 대본 그대로)·T1(한영 혼용+PII)·E1(영어판) + P1(HYP-003 초안, 오너 확인 대상) — 채점 토큰 D1 9·T1 15·E1 10·P1 13. 실측: Soniox 요청 형식 오류 수정(context.general은 [{key,value}] — SDK 소스로 확인) → 잔액 대기 → 충전 완료 / Gladia diarization 400 → 실시간 화자분리 미지원 확정 / Deepgram `nova-3-medical`+ko는 접속 자체가 400(사전 차단 추가), nova-3+ko 정상, 실시간 화자분리는 v1(구세대)뿐·신형 v2는 배치 전용(공식 SDK 주석 확인) — TTS 2종·직접 발화 모두에서 화자 경계 불안정, 최종 판정은 Soniox 동조건 비교로 / 오인식 실측 2건(2제→이제·학회→학교)은 부스팅 기본 용어에 반영. 버그 수정: datalist가 입력값으로 후보를 걸러 모델·목소리가 1개만 보이던 것(→select), subarray 전체 버퍼 전송, 오류 상태가 done으로 덮이던 것, 키 공백 트림. 문서: `scripts/stt_eval/SERVICE_COMPARISON.md` — [확인]/[추정]/[실측] 구분, 비적용 기능(Flux·Redaction·Dictation·Filler Words) 사유 포함 (다음: Soniox 포함 3종 동시 비교 → 서비스 확정 → DECISIONS 기록 · `lib/stt/` Field 이식)
