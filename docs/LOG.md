...
- 08/22 인혁(Claude 세션): **STT 비교 랩 완성 + 첫 실측 — 후보 3→2 (Gladia 탈락), 키 3종 발급·Soniox 충전 완료.** 랩(`apps/stt-lab`, 로컬 전용): 3종 동시 실행(마이크 하나→세 소켓, 한 소리를 셋이 같이 들음) · 3열 라이브 뷰(서비스별 전사·토큰별 채점·첫 응답 ms 나란히) · 대본→음성 Gemini TTS 섹션(화자 2인 한 호출, 모델 목록은 벤더에서 수신, 채점 대본 자동 동기화) · 전사를 확정 문장 단위로 표시(중간 결과는 한 줄 교체) · 마이크 전처리 항상 OFF(스피커 재생 캡처 호환). 평가 자산(`scripts/stt_eval/`): 대본 D1(한국어 7턴)·T1(한영 혼용+PII 8턴)·E1(영어판 7턴)·P1(HYP-003 초안 7턴, 오너 확인 대상) + 채점 토큰 D1 9·T1 15·E1 10·P1 13 + `SERVICE_COMPARISON.md`([확인]/[추정]/[실측] 구분, Flux·Redaction·Dictation·Filler Words 비적용 사유 포함). 실측: 키 3종(Deepgram·Soniox·Gladia) 발급, Soniox 충전 완료 / Soniox 요청 형식 오류 수정(`context.general`은 [{key,value}] — SDK 소스 확인) / **Gladia `diarization` 400 → 실시간 화자분리 미지원 확정(탈락)** / Deepgram `nova-3-medical`+ko 접속 400(랩이 사전 차단), `nova-3`+ko 정상, 실시간 화자분리는 v1뿐·신형 v2는 배치 전용(공식 SDK 확인) — TTS 2종·직접 발화에서 화자 경계 불안정, Soniox 동조건 비교로 판정 예정 / 오인식 실측(2제→이제·학회→학교) 부스팅 용어 반영. 버그 수정: datalist 필터링으로 모델·목소리 후보가 1개만 보이던 것(→select) · subarray 전체 버퍼 전송 · 오류 상태가 종료로 덮이던 것 · 키 공백 트림 · Deepgram 키 만료 401 진단(WS close code 기록 추가). 빌드·타입체크·린트 통과 (다음: Soniox 포함 3종 동시 비교 → 서비스+모델 확정 → DECISIONS 기록 → `lib/stt/` Field 이식)
