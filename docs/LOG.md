...
- 08/22 소정(Claude 세션): **외부 커넥터 4종 질의 명세 신설 (`docs/09`) — 4종 전부 실제 호출로 검증.** openFDA 라벨·CT.gov v2·PubMed E-utilities·Drugs@FDA 모두 **인증 없이 200 확인**, 응답 필드를 실측값으로 고정(추측 0). 호출 중 데모 서사에 직결되는 공개 사실 3건 확보: ① XCOPRI 라벨 `pediatric_use`가 **"소아 안전성·유효성 미확립"**(라벨 개정 2025-09-25) → 12–17 공백이 라벨 원문으로 확정 ② **NCT03961568** SK Life Science, **12세~**, 2019 시작 COMPLETED → 회사가 이미 청소년 방향으로 움직인 공개 증거(HYP-001 검증 축의 실물 근거) ③ **NCT07594158** Ono Pharmaceutical, **2~17세**, 2026-05 RECRUITING → 경쟁사가 소아 구간 진행 중 = 우리 공백이 '미개척'인지 '뒤처짐'인지 판정할 데이터. 캐시·금지경계(개인결합·예측선·FAERS 발생률 비교·라벨 연령 추정)·구현 순서 포함. 미결 4건 명시(경쟁 약물 목록에 levetiracetam 누락 여부 · FAERS 포함 여부 · 문헌 연도 범위 · **커넥터 오너 중복 배정**: WBS 4.1 인혁 ↔ 킥오프 액션 소정) (다음: 미결 확정 후 `connectors/cache.py`부터 구현 — backend는 `[cross]`)
