...
## 4.5 심사 기간 운용 (08/21 — 주최측 기준 반영)

**본선 심사는 09-03 23:59 이후 09-09 발표 전까지 진행되고, 심사위원이 배포본을 직접 사용한다.** 이 기간에 앱이 죽으면 구현 완성도 35점을 확인할 방법이 없어진다("작동하는 앱 URL이 열리지 않으면 구현 완성도를 확인할 수 없습니다").

| 항목 | 요건 | 상태 |
|---|---|---|
| 유료 플랜 전환 | 무료 콜드스타트로 첫 화면이 30초 걸리는 사고 방지 — **9/1까지** | §5와 동일 |
| 매일 자동 리셋 | 여러 심사위원이 만지면 승인 상태가 섞인다 → 리셋 엔드포인트를 **자정 크론**으로 (수동 아님) | 9/1 마감분 |
| 코드 동결 | 9/3 마감 후 **push 금지**. 장애 대응만, 그것도 롤백 우선 | 규칙 |
| 접속 안내 | 로그인이 있으면 접속 방법을 제출 폴더에 적는다 (주최측 명시 요구) | `2_결과물` README |
| 다른 환경 확인 | 마감 전 시크릿 창·모바일에서 URL 재확인 (Chrome 백지 사고 재발 방지 — §1.6) | 9/2 |
